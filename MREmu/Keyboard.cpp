...
KeyboardControl::pkey_t::pkey_t(int key_code, key_source source) : key_code(key_code), source(source) {}

void KeyboardControl::update() {
    for (int i = 0; i < pkey.size(); ++i)
        if (pkey[i].tim.getElapsedTime().asMilliseconds() >= 500) {
            pkey[i].tim.restart();
            if (pkey[i].key_status < VM_KEY_EVENT_REPEAT) pkey[i].key_status++;
        }
}

int KeyboardControl::find_key(int key_code) {
    for (int i = 0; i < pkey.size(); ++i)
        if (pkey[i].key_code == key_code) return i;
    return -1;
}

void KeyboardControl::press_key(int key_code, key_source source) {
    if (key_code == MREMU_KEY_NONE) return;
...
// 按键定义部分保持不变，只有布局改了
struct Keys { char name[20] = ""; int code = 0; };
const Keys keys_imgui[3 * 7] = {
    {"Left S",VM_KEY_LEFT_SOFTKEY},{"UP",VM_KEY_UP},{"Right S",VM_KEY_RIGHT_SOFTKEY},
    {"LEFT",VM_KEY_LEFT},{"OK",VM_KEY_OK},{"RIGHT",VM_KEY_RIGHT},{"C",VM_KEY_CLEAR},
    {"Down",VM_KEY_DOWN},{"Back",VM_KEY_BACK},{"1.,",VM_KEY_NUM1},{"2abc",VM_KEY_NUM2},
    {"3def",VM_KEY_NUM3},{"4ghi",VM_KEY_NUM4},{"5jkl",VM_KEY_NUM5},{"6mno",VM_KEY_NUM6},
    {"7pqrs",VM_KEY_NUM7},{"8tuv",VM_KEY_NUM8},{"9wxyz",VM_KEY_NUM9},{"*",VM_KEY_STAR},
    {"0",VM_KEY_NUM0},{"#",VM_KEY_POUND},
};

const std::map<sf::Keyboard::Key, int> key_to_key = {
    {sf::Keyboard::Up, VM_KEY_UP},{sf::Keyboard::Down, VM_KEY_DOWN},
    {sf::Keyboard::Left, VM_KEY_LEFT},{sf::Keyboard::Right, VM_KEY_RIGHT},
    {sf::Keyboard::Slash, VM_KEY_LEFT_SOFTKEY},{sf::Keyboard::RShift, VM_KEY_RIGHT_SOFTKEY},
    {sf::Keyboard::Enter, VM_KEY_OK},{sf::Keyboard::BackSpace, VM_KEY_CLEAR},
    {sf::Keyboard::Escape, VM_KEY_BACK},{sf::Keyboard::Numpad7, VM_KEY_NUM1},
    {sf::Keyboard::Numpad8, VM_KEY_NUM2},{sf::Keyboard::Numpad9, VM_KEY_NUM3},
    {sf::Keyboard::Numpad4, VM_KEY_NUM4},{sf::Keyboard::Numpad5, VM_KEY_NUM5},
    {sf::Keyboard::Numpad6, VM_KEY_NUM6},{sf::Keyboard::Numpad1, VM_KEY_NUM7},
    {sf::Keyboard::Numpad2, VM_KEY_NUM8},{sf::Keyboard::Numpad3, VM_KEY_NUM9},
    {sf::Keyboard::Divide, VM_KEY_STAR},{sf::Keyboard::Numpad0, VM_KEY_NUM0},
...
bool Keyboard::event(sf::Event& event) {
    ...
    case sf::Event::KeyReleased: {
        ...
        if (el != key_to_key.end()) {
            if (event.type == sf::Event::KeyPressed) kc.press_key(el->second, KeyboardControl::Keyboard);
            else kc.unpress_key(el->second);
        }
        return true;
    }
...
void Keyboard::imgui_keyboard() {
    ...
    for (int i = 0; i < 3 * 7; ++i) {
        ...
        if (ImGui::Button(keys_imgui[i].name, v)) kc.unpress_key(keys_imgui[i].code);
        if (ImGui::IsItemClicked()) kc.press_key(keys_imgui[i].code, KeyboardControl::ImGui);
        if (presed) ImGui::PopStyleColor(3);
    }
    ImGui::End();
}

void Keyboard::draw(sf::RenderTarget* rt) {
    for (int i = 0; i < kc.pkey.size(); ++i) draw_press_key(rt, kc.pkey[i].key_code);
    rt->draw(sp_left);
    rt->draw(sp_right);
}
...
void Keyboard::draw_press_key(sf::RenderTarget* rt, int key) {
    ...
    if (VM_KEY_NUM1 <= key && key <= VM_KEY_NUM9) {
        ...
        switch (key) {
            case VM_KEY_STAR: x = 0, y = 3; break;
            case VM_KEY_NUM0: x = 1, y = 3; break;
            case VM_KEY_POUND: x = 2, y = 3; break;
        }
    }
    if (x != -1) {
        offset_vec = sf::Vector2f(sp_left.getPosition().x + (float)layout_left.off_x,
                                  sp_left.getPosition().y + (float)layout_left.off_y);
        kw = layout_left.kw; kh = layout_left.kh;
    } else {
        offset_vec = sf::Vector2f(sp_right.getPosition().x + (float)layout_right.off_x,
                                  sp_right.getPosition().y + (float)layout_right.off_y);
        kw = layout_right.kw; kh = layout_right.kh;
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j)
                if (key_map_right[i][j] == key) {
                    x = j, y = i;
                    if (!(std::abs(j - 1) == 1 && std::abs(i - 1)) == 1) offset_vec.y += kh / 2.f;
                    else if (i == 2) offset_vec.y += kh;
                    break;
                }
    }
    sf::Vertex v[4];
...
int Keyboard::find_key_by_pos(int px, int py) {
    if (in_box(px, py, sp_left)) {
        ...
        int kpx = (int)((float)(px - x) / layout_left.kw);
        int kpy = (int)((float)(py - y) / layout_left.kh);
    ...
    if (in_box(px, py, sp_right)) {
        ...
        int kpx = (int)((float)(px - x) / layout_right.kw);
        int kpy = (int)((float)(py - y) / layout_right.kh);
        if (kpx < 0 || kpx >= layout_right.cols) return MREMU_KEY_NONE;
        if (kpy < 0 || kpy >= layout_right.rows) return MREMU_KEY_NONE;
        if ((kpx == 0 || kpx == 2) && (kpy == 0 || kpy == 3)) return key_map_right[kpy][kpx];
        {
            float local_y = (float)(py - y);
            kpy = (int)std::floor((local_y - layout_right.kh / 2.f) / layout_right.kh);
            if (kpy >= 0 && kpy < 3 && !(std::abs(kpx - 1) == 1 && std::abs(kpy - 1) == 1)) return key_map_right[kpy][kpx];
        }
    }
...
const char16_t* keys_marks_left[4][3] = {
    {u"1", u"2", u"3"},{u"4", u"5", u"6"},{u"7", u"8", u"9"},{u"*", u"0", u"#"}
};
...
void Keyboard::update_resize(int win_w, int win_h) {
    ...
    // 重新分配区域：D-pad 占上方约 35%，数字键盘占下方约 65%
    int kb_top = screen_y + screen_h + 10;
    int kb_left = (win_w - screen_w) / 2;
    int kb_w = screen_w;
    int kb_h = win_h - kb_top - 14;

    // D-pad 区域（上方）
    int dpad_h = kb_h * 35 / 100;
    sf::IntRect right = { kb_left, kb_top, kb_w, dpad_h };
    // 数字键盘区域（下方）
    sf::IntRect left = { kb_left, kb_top + dpad_h, kb_w, kb_h - dpad_h };

    // ==================== 数字键盘绘制 ====================
    {
        int w = left.width, h = left.height;
        const float GAP = 3.f;
        const float BTN_RC = 4.f;
        const float MAX_KW = 92.f;
        const float MAX_KH = 48.f;

        float kw = std::min(MAX_KW, ((float)w - 2.f * GAP) / 3.f - GAP);
        float kh = std::min(MAX_KH, ((float)h - 2.f * GAP) / 4.f - GAP);
        if (kw < 28.f) kw = 28.f;
        if (kh < 36.f) kh = 36.f;
        ...
        for (int iy = 0; iy < 4; ++iy)
            for (int ix = 0; ix < 3; ++ix) {
                ...
                drawRC(bx, by); drawRC(bx + kw, by); drawRC(bx, by + kh); drawRC(bx + kw, by + kh);
                ...
                if (scale_main > 1.8f) scale_main = 1.8f;
                sp_main.setScale(scale_main, scale_main);
                sp_main.setOrigin((float)sp_main.getTextureRect().width / 2.f, (float)sp_main.getTextureRect().height / 2.f);
                sp_main.setPosition(bx + kw / 2.f - scale_main, by + kh / 2.f - 4.f * scale_main);
    ...
    // ==================== D-pad 绘制（真正画出来！） ====================
    {
        ...
        float kw = (float)w / 3.f;
        float kh = (float)h / 3.f;
        int off_x = 0;
        int off_y = 0;
        layout_right.kw = kw;
        layout_right.kh = kh;
        ...
        // 画方向键背景（圆角方块）
        auto drawDPadBtn = [&](float bx, float by, const char16_t* symbol, bool isCenter) {
            sf::RectangleShape btn(sf::Vector2f(kw - 4.f, kh - 4.f));
            btn.setPosition(bx + 2.f, by + 2.f);
            btn.setFillColor(isCenter ? sf::Color(248, 248, 252) : sf::Color(220, 224, 232));
            btn.setOutlineColor(sf::Color(150, 152, 160));
            btn.setOutlineThickness(1.f);
            frontend_layer_right.draw(btn);

            // 圆角
            sf::CircleShape cr(4.f, 16);
            cr.setFillColor(isCenter ? sf::Color(248, 248, 252) : sf::Color(220, 224, 232));
            cr.setOutlineColor(sf::Color(150, 152, 160));
            cr.setOutlineThickness(1.f);
            auto dpadRC = [&](float cx, float cy) {
                cr.setPosition(cx - 4.f, cy - 4.f);
                frontend_layer_right.draw(cr);
            };
            dpadRC(bx + 2.f, by + 2.f);
            dpadRC(bx + kw - 2.f, by + 2.f);
            dpadRC(bx + 2.f, by + kh - 2.f);
            dpadRC(bx + kw - 2.f, by + kh - 2.f);

            if (symbol) {
                auto tex = u16text_to_texture(symbol, sf::Color(80, 80, 90));
                sf::Sprite sp(tex);
                sp.setPosition(bx + (kw - (float)tex.getSize().x) / 2.f,
                               by + (kh - (float)tex.getSize().y) / 2.f);
                frontend_layer_right.draw(sp);
            }
        };

        drawDPadBtn(0.f, 0.f, u"\u2B06", false); // 上
        drawDPadBtn(0.f, kh, u"\u2B05", false); // 左
        drawDPadBtn(0.f, kh * 2.f, u"\u2B07", false); // 下
        drawDPadBtn(kw * 2.f, kh, u"\u2B95", false); // 右
        drawDPadBtn(kw, kh, u"OK", true); // 中 OK
